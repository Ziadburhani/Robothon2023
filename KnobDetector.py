...
for image_str in images:
    ...
    image = cv2.imread(images_folder + image_str)
    
    # Increase contrast
    contr = increase_contrast(image)
    ...
    if x != 0:
        ...
        knob_center, knob_radius  = detect_knob(image, x, y, w, h)
        
        # Draw both and show the image, just for fun.
        if knob_radius != 0:
            cv2.circle(image, knob_center, 5, (255, 0, 0), -1)
            cv2.circle(image, knob_center, knob_radius, (0, 255, 0), 3)
            show_image(image_str,image)
